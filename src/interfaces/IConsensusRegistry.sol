...
interface IConsensusRegistry {
  ...
  /// @param lastSnapshotCommit The `validatorsCommit` value when the last snapshot of this
  /// validator was made.
  /// @param previousSnapshotCommit The `validatorsCommit` value when the previous snapshot of this
  /// validator was made.
  /// @param latest Validator attributes to read if `validatorsCommit` >
  /// `validator.lastSnapshotCommit`.
  /// @param snapshot Validator attributes to read if `validatorsCommit` >
  /// `validator.previousSnapshot`.
  ...
  /// @param previousSnapshotCommit The `validatorsCommit` value when the previous snapshot was
  /// made.
  ...
  /// @param frequency The number of views between leader changes. If it is 0 then the leader never
  /// rotates.
  /// @param weighted Whether leaders are selectedproportionally to their weight. If false, then the
  /// leader is selected round-robin.
  ...
  event ValidatorKeyChanged(
    address indexed validatorOwner, BLS12_381PublicKey newPubKey, BLS12_381Signature newPoP
  );
  ...
  /// @notice Changes the active status of a validator, determining whether it can participate in
  /// committees.
  /// @dev Only callable by the contract owner or the validator owner.
  /// @dev Verifies that the validator owner exists in the registry.
  /// @param _validatorOwner The address of the owner of the validator whose active status will be
  /// changed.
  ...
  /// @dev Only callable by the contract owner.
  /// @dev Verifies that the validator owner exists in the registry.
  /// @param _validatorOwner The address of the owner of the validator whose leader status will be
  /// changed.
  ...
  /// @dev Only callable by the contract owner or the validator owner.
  /// @dev Verifies that the validator owner exists in the registry.
  /// @param _validatorOwner The address of the owner of the validator whose key and PoP will be
  /// changed.
  ...
  /// @notice Adds a new commit to the validator committee using the current block number plus
  /// delay.
  ...
  /// @notice Returns an array of `ValidatorAttr` structs representing the current validator
  /// committee and the current leader selection configuration.
  ...
  /// @notice Returns an array of `ValidatorAttr` structs representing the pending validator
  /// committee and the pending leader selection configuration.
  /// @dev Collects active and non-removed validators that will form the next committee after the
  /// current commit becomes active.
  ...
  /// @dev Only callable by the contract owner
  /// @param _frequency The number of views between leader changes. If it is 0 then the leader never
  /// rotates.
  /// @param _weighted Whether leaders are selected proportionally to their weight. If false, then
  /// the leader is selected round-robin.
